...
class TestAgentManagerThreadIsolation:
    ...
    def test_thread_isolated_current_agent(self, temp_config_file):
        ...
        thread1 = threading.Thread(target=thread_worker, args=("thread1", "facilitator"))
    ...
    def test_thread_shared_configurations_isolated_current_agent(self, temp_config_file):
        ...
        try:
            def thread_worker(thread_id: str, agent_to_set: str):
                ...
                try:
                    with patch("agents.agent_manager.PAGES_CONFIG_FILE", Path(config_file)):
                        import agents.agent_manager as agent_manager
            ...
            try:
                with patch("agents.agent_manager.PAGES_CONFIG_FILE", Path(temp_config_file)):
                    ...
                    for i in range(5):
                        ...
                        switches.append({"agent": agent, "success": success, "current": current})
    ...
    def test_thread_isolation_with_role_based_access(self, temp_config_file):
        ...
        thread2 = threading.Thread(target=thread_worker, args=("admin_thread", ["admin", "user"]))
        ...
        def thread_worker(thread_id: str, user_roles: list[str]):
            ...
            try:
                with patch("agents.agent_manager.PAGES_CONFIG_FILE", Path(temp_config_file)):
                    ...
                    cache_info_before = agent_manager._extract_agents_from_sections.cache_info()
                    
                    # Make first call - should be a cache miss
                    available_agents1 = agent_manager.get_available_agents(user_roles)
                    cache_info_after_first = agent_manager._extract_agents_from_sections.cache_info()
                    
                    # Make second call with same roles - should be a cache hit
                    available_agents2 = agent_manager.get_available_agents(user_roles)
                    cache_info_after_second = agent_manager._extract_agents_from_sections.cache_info()
